...
plink_base="perceiver/train/"
bed_file = plink_base+"random.bed"
bim_file = plink_base+"random.bim"
fam_file = plink_base+"random.fam"

num_snps = 10000
num_samples = 20000
positions = np.random.choice(range(1000000), size=num_snps, replace=False)
positions.sort()
...
urate_df.to_csv(plink_base+"random_urate.csv", index=False, quoting=csv.QUOTE_NONNUMERIC)

withdrawn_ids = pd.DataFrame(np.random.choice(sample_ids, 10))
withdrawn_ids.to_csv(plink_base+"w12611_20220222.csv", index=False, header=False)